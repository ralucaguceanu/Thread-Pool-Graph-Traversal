...
os_task_t *dequeue_task(os_threadpool_t *tp)
{
	...
	pthread_mutex_lock(&tp->lock);
	while (queue_is_empty(tp) && tp->work_done == 0)
		pthread_cond_wait(&tp->condition_variable, &tp->lock);
...
void wait_for_completion(os_threadpool_t *tp)
{
	...
	// nu mai e nevoie sa asteptam toate threadurile, intrucat am apelat
	// in main pthread_cond_broadcast care elibereaza toate thread-urile
	// si am setat tp->work_done pe true, ca sa se termine programul
